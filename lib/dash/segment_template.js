...
shaka.dash.SegmentTemplate = class {
  ...
  static createStreamInfo(
    ...
    const shallowCopyOfContext = shaka.util.ObjectUtils.shallowCloneObject(
      context
    );
  ...
  static generateSegmentIndexFromDuration_(
    ...
    const computeAvailablePeriodRange = () => {
      return [
        ...
        Math.min(presentationTimeline.getSegmentAvailabilityEnd(), periodEnd)
      ];
    };
  ...
  static createFromTimeline_(context, info, initSegmentReference) {
    ...
    for (const {
      i,
      item: { start, unscaledStart, end }
    } of enum_(info.timeline)) {
